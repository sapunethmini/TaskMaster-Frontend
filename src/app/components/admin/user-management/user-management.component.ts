...
export class UserManagementComponent implements OnInit {
  ...
  loadTeamMemberCounts(): void {
    ...
    const countRequests = this.teams.map(team => 
      this.employeeService.getEmployeesByDepartment(team.departmentId) // Changed to correct method
    );
    ...
    forkJoin(countRequests).subscribe({
      next: (counts) => {
        ...
        console.log('Team member counts loaded:', this.teams);
      },
      error: (error) => {
        console.error('Error fetching team member counts:', error);
        this.isLoading = false;
        // Set loading to false even on error to show the cards
        // Member counts will remain 0 but users can still click to view teams
      }
    });
  }
